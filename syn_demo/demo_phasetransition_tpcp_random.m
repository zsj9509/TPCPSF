...
tpcp_result_Lr = zeros(ds,ds);

lambda = 1/sqrt(n3*max(n1,n2));
opts.tol = 1e-8; % LuCY's
% opts.tol = 1e-7; % Xue's
opts.mu = 1e-3;
opts.rho = 1.1;
opts.DEBUG = 0;

mkdir('./syn_tpcp_result');
...
for rank_ratio = 0.01:0.01:0.5
    ...
    for p = 0.01:0.01:0.5
        ...
        %--------------------------- tpcp-----------------
        [L_tpcp,~] = trpca_tnn(M,lambda,opts);
        Lr_tpcp = norm(L(:)-L_tpcp(:))/norm(L(:));
        tpcp_result_Lr(num1,num2) = Lr_tpcp;

    end
    save(strcat('./syn_tpcp_result/rank',num2str(num1),'.mat'),'tpcp_result_Lr');
end

save ./syn_tpcp_result/tpcp_random.mat tpcp_result_Lr
